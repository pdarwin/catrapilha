...
import { normalizedTagGroups } from "./TagReplacements";
        ...
        if (!metadata.downloadLinks || metadata.downloadLinks.length === 0) {
          ...
          const item = {
            ...
            filename: `IBPA ${page} - ${processDescription(
              ...
              metadata.authorship
                ? metadata.authorship
                    .replace(/\s*\/\s*/g, "-") // Replaces "/" with "-" and removes extra spaces around it
                    .replace(/:/g, "")
                : "Unknown Author"
            }.${
              metadata.imagePath
...
const getCategoriesFromTags = metadata => {
  ...
  for (const group of normalizedTagGroups) {
    for (const tag of group.tags) {
      tagReplacements[tag] = group.replacements;
    }
  }
  ...
  if (
    ...
    tags.some(tag => ["Encerramento"].includes(tag))
  ) {
  ...
  const ceremonyTags = [
    ...
    "Cerimônia de Entrega do Novo Centro de Atenção Psicossocial Álcool e Drogas tipo 4 (Caps 4)",
  ...
  if (
    tags.some(tag =>
      [
        "Alvará",
        "Alvará de funcionamento para Empresa Kley Hertz",
  ...
  if (
    !tags.some(tag =>
      ["Jornada de Estudos em Defesa do Adolescente"].includes(tag),
    ) &&
    (orTags.some(tag =>
  ...
  if (
    tags.some(tag =>
      ["Transporte Público", "Transporte Coletivo"].includes(tag),
    )
  ) {
    categories.push("Public transport in Porto Alegre");
  }

  const isCarrisBus = tags.some(tag =>
    ["Carris", "Papai e Mamãe Noel da Carris"].includes(tag),
  );
  ...
  if (tags.some(tag => ["Propaganda Irregular"].includes(tag))) {
  ...
  if (
    tags.some(tag =>
      [
        "Aula aberta",
        "Oficina",
        "Oficina de Inovação Urbana Resíduos Sólidos e Limpeza Urbana",
  ...
  if (
    tags.some(tag =>
      [
        ...
        "Autuação",
        "Blitz",
        "Destruição de Cigarros Ilegais",
        "Fiscalização",
        "Flagrante",
        "Homologação",
        "Interdição",
  ...
  if (tags.includes("Destruição de Cigarros Ilegais")) {
    categories.push(
      "Anti-smoking",
      "Cigarette smuggling",
      "Cigarettes in Brazil",
      "Customs services of Brazil",
    );
  }

  if (tags.includes("Relatório de Balneabilidade")) {
    categories.push(
      "Beaches of Porto Alegre",
      "Environmental problems in Rio Grande do Sul",
      "Reports",
  ...
  if (
    ...
    tags.some(tag =>
      ["Aedes aegypti", "Sala de monitoramento do Aedes da SES/RS"].includes(
        tag,
      ),
    )
  ) {
  ...
  if (
    tags.some(tag =>
      ["Coleta Seletiva", "Novo Layout Caminhões Coleta Seletiva"].includes(
        tag,
      ),
    )
  ) {
  ...
  if (tags.some(tag => ["Compostagem", "Reciclagem"].includes(tag))) {
  ...
  if (
    tags.some(tag =>
      [
        ...
        "Unidade de Destino Certo Diário de Notícias",
        "Unidade de Destino Certo Fátima Pinto",
        "Unidade de Destino Certo Humaitá",
        "Unidade de Destino Certo Princesa Isabel",
    ...
    categories.push("Recycling stations in Porto Alegre");
  }
  ...
  if (
    !tags.some(tag =>
      [
        "Assembleia de Verão 2019 da Famurs",
  ...
  if (
    ...
    tags.some(tag =>
      [
        "Campeonato",
        "Concurso",
        "Primeiro Quiz Cultural na Ilha da Pintada",
      ].includes(tag),
    )
  ) {
  ...
  if (
    ...
    tags.some(tag => ["Saúde Prisional", "Prisão"].includes(tag))
  ) {
  ...
  if (tags.some(tag => ["Calçada", "Calçada Sensorial"].includes(tag))) {
    categories.push("Sidewalks in Porto Alegre");
  }
  ...
  if (
    !tags.includes("Exposição Deus Momo Vem Aí") &&
    tags.some(tag => ["Museu de Porto Alegre Joaquim Felizardo"].includes(tag))
  ) {
    categories.push("Museu de Porto Alegre Joaquim Felizardo");
  }
  ...
  if (
    !tags.includes("Auditório da Associação dos Delegados de Polícia") &&
    tags.some(tag => ["Bairro Azenha"].includes(tag))
  ) {
    categories.push("Azenha");
  }
  ...
  if (tags.some(tag => ["Nova Rótula da Avenida Icaraí"].includes(tag))) {
    categories.push("Avenida Icaraí");
    categories.push("Roundabouts in Brazil");
  }
  ...
  if (
    !tags.includes("Praça Coinma") &&
    tags.includes("Bairro Jardim Itu-Sabará")
  ) {
    categories.push("Jardim Itu-Sabará");
  }
  ...
  if (
    !tags.some(tag =>
      ["Casa da Estrela", "Praça Mafalda Veríssimo", "uMov.me"].includes(tag),
    ) &&
  ...
  if (
    !tags.includes("Praça Major Joaquim Queiróz") &&
    tags.includes("Bairro Santana")
  ) {
    categories.push("Santana (Porto Alegre)");
  }
  ...
  if (
    !tags.includes("Quilombo Família Silva") &&
    tags.includes("Bairro Três Figueiras")
  ) {
    categories.push("Três Figueiras");
  }

  if (
    !tags.some(tag => ["Parque Knijnik"].includes(tag)) &&
    tags.includes("Vila Nova")
  ) {
    categories.push("Vila Nova (Porto Alegre)");
  }

  if (
    !tags.some(tag => ["Bairro Rubem Berta"].includes(tag)) &&
    tags.includes("Zona Norte")
  ) {
    categories.push("Zona Norte (Porto Alegre)");
  }
  ...
  if (tags.includes("Porto Alegre Capital das Microcervejarias Artesanais")) {
    categories.push("Breweries in Porto Alegre", "Microbreweries in Brazil");
  }
  ...
  if (tags.some(tag => ["Transmissão de Cargo"].includes(tag))) {
  ...
  if (
    !tags.includes("Exposição SobrePoa") &&
    tags.some(tag => ["exposição", "Exposição Fotográfica"].includes(tag))
  ) {
  ...
  if (
    !tags.includes("Exposição SobrePoa") &&
    tags.some(tag => ["Exposição Fotográfica"].includes(tag))
  ) {
    categories.push("Photography exhibitions in Brazil");
  }
  ...
  if (
    !tags.includes("Evento Mulheres em Ação") &&
    tags.some(tag => ["Espetáculo", "Show de Talentos"].includes(tag))
  ) {
  ...
  if (
    tags.some(tag =>
      ["Calor", "clima", "Estiagem", "Previsão do Tempo", "Tempo"].includes(
        tag,
      ),
    )
  ) {
  ...
  if (
    tags.some(tag =>
      [
        "Fórum da Liberdade",
        "Conferência",
        "Evento Caminhos para a Inovação",
      ].includes(tag),
    )
  ) {
  ...
  if (tags.some(tag => ["Festa à Fantasia"].includes(tag))) {
    categories.push("Costume parties", "Parties in Brazil");
    tags.push("Carnaval");
  }
  ...
  if (tags.some(tag => ["Sarau"].includes(tag))) {
    categories.push(
      "Cultural events in Rio Grande do Sul",
      `${getYear(metadata.humanReadableDate)} cultural events in Brazil`,
    );
  }
  ...
  const year = getYear(metadata.humanReadableDate);

  const eventCategories = [
    `${year} festivals in Porto Alegre`,
    `Carnival of Porto Alegre ${year}`,
    `Christmas ${year} in Porto Alegre`,
    `New Year ${year} in Brazil`,
  ];
  ...
  const hasEventCategory = categories.some(category =>
    eventCategories.includes(category),
  );

  const hasEventTag = tags.some(tag => eventTags.includes(tag));

  if (
    !(hasEventCategory || hasEventTag) &&
    (isExecutiveTag ||
      isMeetingTag ||
      ...
      orTags.some(tag =>
        [
          "Ação da Coordenação de Educação da EPTC",
      ...
      tag =>
        ...
        tags.some(tag =>
          [
            ...
            "5ª Edição da Cerimônia de Lavagem das Escadarias do Paço",
            "5ª edição do Pedal das Chaminés",
